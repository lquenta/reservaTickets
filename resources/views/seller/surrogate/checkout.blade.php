...
<div class="max-w-2xl mx-auto mb-6 sm:mb-8 px-1">
    <a href="{{ route($flow->checkoutSuccessRoute) }}" class="text-sm text-[#ff2daa] hover:text-[#39ff14]">← Volver a eventos</a>
</div>
...
<div class="max-w-2xl mx-auto px-1" x-data="checkoutTimer({{ $reservation->expires_at->timestamp * 1000 }})" x-init="start()">
    ...
    <div class="rounded-2xl border border-purple-900/50 bg-black/60 backdrop-blur px-4 py-6 sm:p-8 md:p-10 space-y-8">
        ...
        <div class="rounded-2xl border-2 border-[#ff2daa]/60 bg-[#ff2daa]/10 p-6">
            <h2 class="font-display text-xl font-bold text-[#ff2daa] tracking-wider mb-4">ENTRADAS DEL CLIENTE</h2>
            <ul class="space-y-3">
        ...
        <form method="POST" action="{{ $confirmRoute }}" enctype="multipart/form-data" class="space-y-5 pt-2">
            ...
            <div class="rounded-xl border-2 border-amber-500/50 bg-amber-900/20 p-4">
                <label class="flex items-start gap-3 text-sm text-amber-100 cursor-pointer">
                    <input type="checkbox" name="seller_delivery_responsibility" value="1" required class="mt-1 rounded border-amber-600/50 text-[#ff2daa] focus:ring-[#39ff14] bg-black/60 size-5">
