...
<div class="rounded-2xl border-2 border-violet-200/60 dark:border-violet-700/50 bg-white dark:bg-slate-800/80 overflow-hidden shadow-lg">
    <div class="px-5 py-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50">
        <h2 class="text-lg font-semibold text-slate-800 dark:text-white">Listado (orden de aparición en el slider)</h2>
    </div>
    <div class="p-6">
        @forelse($members as $member)
            <div class="flex items-center gap-4 py-4 border-b border-slate-200 dark:border-slate-700 last:border-0">
                <div class="w-20 h-20 rounded-xl overflow-hidden bg-slate-200 dark:bg-slate-700 shrink-0 ring-2 ring-slate-300 dark:ring-slate-600">
                    <img src="{{ asset('storage/'.$member->photo_path) }}" alt="{{ $member->name ?: 'Integrante' }}" class="w-full h-full object-cover">
                </div>
                ...
                <div class="flex items-center gap-2 shrink-0">
                    ...
                    <form action="{{ route('admin.team-members.destroy', $member) }}" method="POST" onsubmit="return confirm('¿Eliminar este integrante?');">
                        @csrf
                        @method('DELETE')
        ...
        @empty
            <p class="py-12 text-center text-slate-500 dark:text-slate-400">No hay integrantes. <a href="{{ route('admin.team-members.create') }}" class="text-[#e50914] hover:underline">Añade el primero</a> para que aparezcan en el slider de Quiénes somos.</p>
        @endforelse
    </div>
</div>
@endsection
