...
<div class="max-w-4xl mx-auto mb-6 rounded-2xl border-2 border-violet-300 dark:border-violet-600 bg-violet-50 dark:bg-violet-900/30 p-4">
    ...
    @if($client->isGuest())
        <p class="text-sm text-violet-800 dark:text-violet-300 mt-1">Invitado temporal — entrega manual de tickets · {{ $client->phone ?: 'sin teléfono' }}</p>
    @else
...
<div class="max-w-2xl mx-auto mb-6 sm:mb-8 px-1">
    <div class="flex flex-wrap items-center justify-center gap-x-2 gap-y-2 sm:gap-x-4">
        <div class="flex items-center gap-2 min-w-0">
            <span class="flex h-9 w-9 sm:h-10 sm:w-10 shrink-0 items-center justify-center rounded-full bg-[#ff2daa] text-xs sm:text-sm font-bold text-white">1</span>
            <span class="text-xs sm:text-sm font-medium text-[#ff2daa]"><span class="sm:hidden">Butacas / datos</span><span class="hidden sm:inline">Elige butacas / datos</span></span>
        </div>
        <div class="h-px w-8 sm:w-12 bg-purple-900/50 shrink-0" aria-hidden="true"></div>
        <div class="flex items-center gap-2 min-w-0">
            <span class="flex h-9 w-9 sm:h-10 sm:w-10 shrink-0 items-center justify-center rounded-full border-2 border-purple-900/60 bg-black/40 text-xs sm:text-sm font-medium text-white/60">2</span>
...
@if(!empty($sectionsData))
    ...
    <div class="mx-auto w-full min-w-0 max-w-full px-2 sm:max-w-4xl sm:px-0" x-data="reservationSections({
        ...
        <h1 class="font-display text-2xl sm:text-3xl font-bold text-[#ff2daa] tracking-widest mb-2">CHECKOUT — PASO 1</h1>
        ...
        <form id="reservation-form-sections" method="POST" action="{{ $storeRoute }}" class="space-y-8">
            ...
            <template x-if="hasCustomLayout()">
                <div class="rounded-2xl border border-purple-900/50 bg-black/60 backdrop-blur px-4 py-5 sm:p-6">
                    ...
                    <div class="mb-3 flex flex-wrap items-center justify-center gap-2 rounded-lg border border-purple-900/50 bg-black/50 px-2 py-2 text-white shadow-inner">
                        <span class="w-11 text-center font-mono text-xs tabular-nums text-white/90" x-text="layoutZoomPercent() + '%'"></span>
                        <button type="button" class="inline-flex h-9 min-w-[2.25rem] items-center justify-center rounded-lg border border-purple-800/60 bg-black/60 px-2 text-lg font-semibold text-white hover:bg-purple-950/50 active:scale-95" @click.prevent="layoutZoomOut()" title="Alejar plano" aria-label="Alejar plano">−</button>
                        <button type="button" class="inline-flex h-9 items-center justify-center rounded-lg border border-purple-800/60 bg-black/60 px-3 text-xs font-semibold text-white hover:bg-purple-950/50 active:scale-95" @click.prevent="layoutZoomResetFit()" title="Encajar al espacio" aria-label="Encajar plano al espacio">Encajar</button>
                        <button type="button" class="inline-flex h-9 min-w-[2.25rem] items-center justify-center rounded-lg border border-purple-800/60 bg-black/60 px-2 text-lg font-semibold text-white hover:bg-purple-950/50 active:scale-95" @click.prevent="layoutZoomIn()" title="Acercar plano" aria-label="Acercar plano">+</button>
                    ...
                    <div class="mt-5 mb-4 w-full max-w-xl mx-auto">
                        ...
                        <select id="layout-section-view-select"
                                ...
                                class="w-full rounded-xl border-2 border-purple-900/50 bg-black/70 px-4 py-3.5 sm:py-4 text-center text-base sm:text-lg font-semibold text-white shadow-inner focus:outline-none focus:ring-2 focus:ring-[#39ff14] focus:border-[#39ff14] cursor-pointer appearance-none bg-[length:1.25rem] bg-[right_0.75rem_center] bg-no-repeat pr-10"
                    ...
                    <div x-ref="layoutViewport"
                         ...
                         class="relative w-full min-h-[200px] max-h-[min(78dvh,900px)] touch-manipulation overflow-auto overscroll-contain rounded-xl border border-purple-900/40 bg-[radial-gradient(circle,_rgba(255,255,255,0.12)_1px,_transparent_1px)] bg-[size:16px_16px] p-2 sm:max-h-[min(86vh,90dvh)]">
                                ...
                                <template x-for="el in sortedLayoutElements" :key="el.id">
                                    <div class="absolute isolate" :style="layoutElementWrapperStyle(el) + 'pointer-events:none;'">
                                        ...
                                        <template x-if="layoutElType(el) === 'stage'">
                                            <div class="absolute inset-0 z-0 flex items-center justify-center rounded-md border border-[#ff2daa]/40 bg-[#ff2daa] px-0.5 text-white shadow-md pointer-events-none overflow-hidden"
            ...
            @if($hasCustomLayoutBlade && $seatSectionsForLegend->isNotEmpty())
                <div class="rounded-2xl border border-purple-900/50 bg-black/60 backdrop-blur px-4 py-5 sm:p-6">
                    <h2 class="text-lg font-semibold text-[#ff2daa] mb-1">Precios por sector (plano)</h2>
                    <p class="text-white/60 text-sm mb-4">La selección es en el plano de arriba; aquí solo referencia de precio por zona.</p>
                    <ul class="divide-y divide-purple-900/30 rounded-xl border border-purple-900/40 overflow-hidden">
            ...
            @foreach($sectionsData as $section)
                ...
                @endif
                <div class="rounded-2xl border border-purple-900/50 bg-black/60 backdrop-blur px-4 py-5 sm:p-6">
                    <h2 class="text-lg font-semibold text-[#ff2daa] mb-1">{{ $section['name'] }}</h2>
                    @if($isHonoredGuest)
                    ...
                    @elseif(isset($section['price']) && $section['price'] !== null && $section['price'] > 0)
                        <p class="text-white/60 text-sm mb-4">
                            ...
                            @if(isset($section['list_price']) && $section['list_price'] !== null && (float) $section['list_price'] > (float) $section['price'])
                                ...
                                <span class="inline-flex items-center rounded bg-[#39ff14]/90 px-1.5 py-0.5 text-[10px] font-bold uppercase text-[#0d061b] mr-1">Preventa</span>
                            @endif
                        ...
                        <div class="flex flex-col gap-2 items-center section-seat-plan" x-show="!hasCustomLayout()" style="--section-seat-size: clamp(0.875rem, 6vw, 2.5rem);">
                            ...
                            <div class="flex gap-2 items-end flex-nowrap mb-1">
                                ...
                                <div class="flex items-end gap-2 shrink-0 min-w-0" style="width: calc(var(--section-seat-size) * {{ $sectionMaxCols }} + ({{ $sectionMaxCols }} - 1) * 0.5rem);">
                                    ...
                                    <div class="flex-1 flex flex-col items-center justify-end gap-0.5 min-w-0 pb-0.5">
                                        <div class="w-full rounded-sm bg-[#ff2daa] min-h-[3px]" style="height: 3px;" role="img" aria-label="Línea de escenario"></div>
                                        <span class="text-[10px] font-medium text-[#39ff14] uppercase tracking-wider">ESCENARIO</span>
                                    </div>
                                    <div class="flex flex-col items-center justify-center rounded-lg border border-amber-600/50 bg-amber-900/20 shrink-0 py-1 px-1.5 gap-0.5" style="width: var(--section-seat-size); min-height: var(--section-seat-size);" role="img" aria-label="Parlante">
                                        <span class="text-[9px] sm:text-[10px] font-semibold text-amber-400/90 uppercase leading-tight"><span class="sm:hidden">P</span><span class="hidden sm:inline">PARLANTE</span></span>
                                    </div>
                            ...
                            @foreach($seatsByRow as $row => $rowSeats)
                                ...
                                <div class="flex gap-2 items-center justify-center">
                                    <span class="shrink-0 flex items-center justify-center font-bold text-[#ff2daa]" style="width: var(--section-seat-size); height: var(--section-seat-size); font-size: min(0.875rem, var(--section-seat-size)); line-height: 1;">{{ $rowLetter }}</span>
                    ...
                    @else
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <select name="section_quantities[{{ $section['id'] }}]" x-model.number="sectionQuantities[{{ $section['id'] }}]" class="rounded-xl border border-purple-900/50 bg-black/60 px-4 py-2 text-white">
            ...
            <div class="rounded-2xl border border-purple-900/50 bg-black/60 backdrop-blur px-4 py-5 sm:p-6 space-y-4">
                <p class="block text-sm font-medium text-white/80">Nombres en los tickets</p>
                <div class="flex flex-col gap-3 sm:flex-row sm:gap-6">
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="single_name" value="1" x-model="singleName" class="text-[#ff2daa] focus:ring-[#39ff14] bg-black/60">
                        <span class="text-white/70">Un nombre para todos</span>
                    </label>
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="single_name" value="0" x-model="singleName" class="text-[#ff2daa] focus:ring-[#39ff14] bg-black/60">
                ...
                <div x-show="singleName === '1' || singleName === true">
                    ...
                    <input id="holder_name" type="text" name="holder_name" x-model="holderName" maxlength="255"
                           class="w-full rounded-xl border border-purple-900/50 bg-black/60 px-4 py-3 text-white">
                </div>
                <div x-show="singleName === '0' || singleName === false" x-cloak>
                    ...
                    <template x-for="(item, index) in ticketItems" :key="index">
                        <div class="mb-4 p-4 rounded-xl border border-purple-900/30 bg-black/40">
                            <label :for="'holder_name_'+ (index+1)" class="block text-sm font-medium text-[#ff2daa] mb-1" x-text="'Ticket ' + (index+1) + ' — ' + item.label"></label>
                            <input :id="'holder_name_'+ (index+1)" type="text" :name="'holder_name_' + (index+1)" maxlength="255"
                                   ...
                                   class="w-full rounded-xl border border-purple-900/50 bg-black/60 px-4 py-3 text-white placeholder-white/40">
                        </div>
                    </template>
                </div>
            </div>
            ...
            <button type="submit"
                    class="w-full rounded-xl bg-[#ff2daa] px-5 py-4 text-white font-bold hover:bg-pink-600 transition disabled:opacity-50"
        ...
        function reservationSections(config) {
            ...
            return {
                ...
                layoutSeatFaceStyle(el) {
                    ...
                    if (selected) {
                        shadow = '0 0 0 2px #fff, 0 0 0 5px rgba(255,45,170,0.95)';
                    }
                    if (selected) {
                        return `opacity:1;background-color:#ff2daa;border-color:#fecaca;color:#ffffff;box-shadow:${shadow};`;
                    }
                    if (!passFilter) {
                ...
                sectionSeatClass(seatId, availableIds) {
                    ...
                    if (selected) return 'bg-[#ff2daa] text-white ring-2 ring-white';
...
@elseif(empty($seats))
    ...
    <div class="max-w-2xl mx-auto px-1" x-data="{ quantity: {{ old('quantity', 1) }}, singleName: {{ old('single_name', true) ? 'true' : 'false' }} }">
        ...
        <form method="POST" action="{{ $storeRoute }}" class="rounded-2xl border border-purple-900/50 bg-black/60 backdrop-blur px-4 py-6 sm:p-8 md:p-10 space-y-6">
            @csrf
            <input type="hidden" name="event_id" value="{{ $event->id }}">
            @error('event_id')<p class="text-sm text-red-400">{{ $message }}</p>@enderror
            ...
            <div>
                ...
                <select name="quantity" x-model.number="quantity" required
                        class="w-full rounded-xl border border-purple-900/50 bg-black/60 px-4 py-3 text-white focus:ring-2 focus:ring-[#39ff14] focus:border-[#39ff14]">
                    @for($i = 1; $i <= 4; $i++)
            ...
            <div x-show="singleName === '1' || singleName === true">
                ...
                <input id="holder_name" type="text" name="holder_name" value="{{ old('holder_name') }}" maxlength="255"
                       class="w-full rounded-xl border border-purple-900/50 bg-black/60 px-4 py-3 text-white placeholder-white/40 focus:ring-2 focus:ring-[#39ff14] @error('holder_name') border-red-500 @enderror">
                ...
                @for($i = 1; $i <= 4; $i++)
                    <div class="mb-4" x-show="quantity >= {{ $i }}">
                        ...
                        <input id="holder_name_{{ $i }}" type="text" name="holder_name_{{ $i }}" value="{{ old("holder_name_{$i}") }}" maxlength="255"
                               class="w-full rounded-xl border border-purple-900/50 bg-black/60 px-4 py-3 text-white placeholder-white/40 focus:ring-2 focus:ring-[#39ff14]">
            ...
            <button type="submit" class="w-full rounded-xl bg-[#ff2daa] px-5 py-4 text-white font-bold hover:bg-pink-600 transition">
                Continuar al paso 2 — Comprobante de pago
            </button>
        </form>
    </div>
    ...
    <div class="mx-auto w-full min-w-0 max-w-full px-2 sm:max-w-4xl sm:px-0"
         x-data="{
            ...
            layoutSeatFaceStyleSimple(el) {
                ...
                let shadow = '0 1px 2px rgba(0,0,0,0.35)';
                if (selected) shadow = '0 0 0 2px #fff, 0 0 0 5px rgba(255,45,170,0.95)';
                if (selected) {
                    return `background-color:#ff2daa;border-color:#fecaca;color:#fff;box-shadow:${shadow};`;
                }
                if (!can) {
        ...
        <form method="POST" action="{{ $storeRoute }}" class="space-y-6"
            ...
            <template x-if="hasCustomLayout()">
                <div class="rounded-2xl border border-purple-900/50 bg-black/60 backdrop-blur px-4 py-5 sm:p-6">
                    ...
                    <div class="mb-3 flex flex-wrap items-center justify-center gap-2 rounded-lg border border-purple-900/50 bg-black/50 px-2 py-2 text-white shadow-inner">
                        ...
                        <button type="button" class="inline-flex h-9 min-w-[2.25rem] items-center justify-center rounded-lg border border-purple-800/60 bg-black/60 px-2 text-lg font-semibold text-white hover:bg-purple-950/50 active:scale-95" @click.prevent="layoutZoomOutSimple()" title="Alejar plano" aria-label="Alejar plano">−</button>
                        <button type="button" class="inline-flex h-9 items-center justify-center rounded-lg border border-purple-800/60 bg-black/60 px-3 text-xs font-semibold text-white hover:bg-purple-950/50 active:scale-95" @click.prevent="layoutZoomResetFitSimple()" title="Encajar al espacio" aria-label="Encajar plano">Encajar</button>
                        <button type="button" class="inline-flex h-9 min-w-[2.25rem] items-center justify-center rounded-lg border border-purple-800/60 bg-black/60 px-2 text-lg font-semibold text-white hover:bg-purple-950/50 active:scale-95" @click.prevent="layoutZoomInSimple()" title="Acercar plano" aria-label="Acercar plano">+</button>
                        <span class="hidden text-[10px] text-white/45 sm:inline">Ctrl + rueda</span>
                    </div>
                    <div x-ref="layoutViewport"
            ...
            <div class="rounded-2xl border border-purple-900/50 bg-black/60 backdrop-blur px-4 py-5 sm:p-6" x-show="!hasCustomLayout()">
                ...
                <div class="w-full seat-plan-grid overflow-hidden" style="--cols: {{ $maxCols }}; --seat-size: clamp(0.875rem, calc((100vw - {{ $paddingVw }}rem - {{ $labelW }}rem - {{ $gapLabel }}rem - (var(--cols) - 1) * {{ $gapSeat }}rem) / var(--cols)), 3rem);">
                    ...
                    <div class="flex gap-3 items-end flex-nowrap mb-1">
                        ...
                        <div class="flex items-end gap-2 shrink-0 min-w-0" style="width: calc(var(--seat-size) * {{ $maxCols }} + ({{ $maxCols }} - 1) * 0.5rem);">
                            ...
                            <div class="flex-1 flex flex-col items-center justify-end gap-0.5 min-w-0 pb-0.5">
                                <div class="w-full rounded-sm bg-[#ff2daa] min-h-[3px]" style="height: 3px;" role="img" aria-label="Línea de escenario"></div>
                                <span class="text-[10px] sm:text-xs font-medium text-[#39ff14] uppercase tracking-wider">ESCENARIO</span>
                            </div>
                            <div class="flex flex-col items-center justify-center rounded-lg border border-amber-600/50 bg-amber-900/20 shrink-0 py-1 px-1.5 gap-0.5" style="width: var(--seat-size); min-height: var(--seat-size);" role="img" aria-label="Parlante">
                                <span class="text-[9px] sm:text-[10px] font-semibold text-amber-400/90 uppercase leading-tight"><span class="sm:hidden">P</span><span class="hidden sm:inline">PARLANTE</span></span>
                            </div>
                    ...
                    @foreach($seatsByRow as $row => $rowSeats)
                        ...
                        <div class="flex gap-3 items-center justify-center flex-nowrap">
                            <span class="seat-plan-label flex shrink-0 items-center justify-center rounded-lg border border-purple-900/50 bg-black/40 font-bold text-[#ff2daa]" style="width: var(--seat-size); height: var(--seat-size); font-size: min(0.875rem, var(--seat-size)); line-height: 1;" aria-label="Fila {{ $rowLetter }}">{{ $rowLetter }}</span>
                                ...
                                @foreach($rowSeats as $seat)
                                    <button type="button"
                                            ...
                                            :class="{
                                                ...
                                                'bg-[#ff2daa] text-white ring-2 ring-white': isSelected({{ $seat->id }}),
            ...
            <div class="rounded-2xl border border-purple-900/50 bg-black/60 backdrop-blur px-4 py-5 sm:p-6 space-y-4">
                ...
                <div x-show="singleName === '0' || singleName === false" x-cloak>
                    ...
                    <template x-for="(id, i) in selectedIds" :key="id">
                        <div class="mb-4 p-4 rounded-xl border border-purple-900/30 bg-black/40 space-y-3">
                            <p class="text-sm font-medium text-[#ff2daa]" x-text="'Ticket ' + (i+1)"></p>
                            <div>
                            ...
                            <div>
                                ...
                                <select :id="'seat_for_'+ (i+1)" :name="'seat_for_' + (i+1)" required
                                        ...
                                        class="w-full rounded-xl border border-purple-900/50 bg-black/60 px-4 py-3 text-white focus:ring-2 focus:ring-[#39ff14]">
            ...
            <button type="submit" class="w-full rounded-xl bg-[#ff2daa] px-5 py-4 text-white font-bold hover:bg-pink-600 transition disabled:opacity-50 disabled:cursor-not-allowed"
