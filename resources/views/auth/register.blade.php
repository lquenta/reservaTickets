...
<div class="max-w-md mx-auto">
    <div class="rounded-2xl border border-fuchsia-900/50 bg-black/60 backdrop-blur p-8 md:p-10">
        <h1 class="font-display text-3xl font-bold text-[#e11d8a] tracking-widest mb-2">CREAR CUENTA</h1>
        ...
        <form method="POST" action="{{ route('register') }}" class="space-y-4">
            ...
            <div>
                ...
                <input id="name" type="text" name="name" value="{{ old('name') }}" required autofocus autocomplete="name" maxlength="255"
                       class="w-full rounded-xl border border-fuchsia-900/50 bg-black/60 px-4 py-3 text-white placeholder-white/40 focus:ring-2 focus:ring-[#22d3ee] focus:border-[#22d3ee] @error('name') border-red-500 @enderror"
                       pattern="[\p{L}\s\-\.']+">
                @error('name')
            ...
            <div>
                ...
                <input id="email" type="email" name="email" value="{{ old('email') }}" required autocomplete="email" maxlength="255"
                       class="w-full rounded-xl border border-fuchsia-900/50 bg-black/60 px-4 py-3 text-white placeholder-white/40 focus:ring-2 focus:ring-[#22d3ee] focus:border-[#22d3ee] @error('email') border-red-500 @enderror">
                @error('email')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <input id="email_confirmation" type="email" name="email_confirmation" value="{{ old('email_confirmation') }}" required autocomplete="email" maxlength="255"
                       class="w-full rounded-xl border border-fuchsia-900/50 bg-black/60 px-4 py-3 text-white placeholder-white/40 focus:ring-2 focus:ring-[#22d3ee] @error('email_confirmation') border-red-500 @enderror">
            ...
            <div>
                ...
                <input id="ci" type="text" name="ci" value="{{ old('ci') }}" required maxlength="15"
                       class="w-full rounded-xl border border-fuchsia-900/50 bg-black/60 px-4 py-3 text-white placeholder-white/40 focus:ring-2 focus:ring-[#22d3ee] @error('ci') border-red-500 @enderror"
                       pattern="[0-9\-]+">
                @error('ci')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input id="phone" type="text" name="phone" value="{{ old('phone') }}" required maxlength="20"
                       class="w-full rounded-xl border border-fuchsia-900/50 bg-black/60 px-4 py-3 text-white placeholder-white/40 focus:ring-2 focus:ring-[#22d3ee] @error('phone') border-red-500 @enderror"
                       pattern="[0-9\s+\-]+">
                @error('phone')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input id="password" type="password" name="password" required autocomplete="new-password" minlength="8" maxlength="255"
                       class="w-full rounded-xl border border-fuchsia-900/50 bg-black/60 px-4 py-3 text-white placeholder-white/40 focus:ring-2 focus:ring-[#22d3ee] @error('password') border-red-500 @enderror">
                @error('password')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <button type="submit" class="w-full rounded-xl bg-[#e11d8a] px-5 py-3.5 text-white font-bold hover:bg-fuchsia-700 transition">
                Registrarse
            </button>
        </form>

        <p class="mt-6 text-center text-white/70">
            ¿Ya tienes cuenta? <a href="{{ route('login') }}" class="font-medium text-[#e11d8a] hover:text-[#22d3ee]">Iniciar sesión</a>
        </p>
    </div>
</div>
@endsection
