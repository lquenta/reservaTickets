...
<div class="rounded-2xl border border-purple-900/50 bg-black/60 backdrop-blur px-4 py-5 sm:p-6 mb-8"
     x-data="checkoutLayoutMapReadonly(@js($map))">
    <h2 class="font-display text-lg font-bold text-[#ff2daa] tracking-wider mb-1 text-center">Plano — butacas elegidas</h2>
    ...
    <div class="mb-3 flex flex-wrap items-center justify-center gap-2 rounded-lg border border-purple-900/50 bg-black/50 px-2 py-2 text-white shadow-inner">
        ...
        <button type="button" class="inline-flex h-9 items-center justify-center rounded-lg border border-purple-800/60 bg-black/60 px-3 text-xs font-semibold text-white hover:bg-purple-950/50 active:scale-95" @click.prevent="layoutZoomResetFit()" title="Encajar al espacio" aria-label="Encajar plano">Encajar</button>
    ...
    <div x-ref="layoutViewport"
         ...
         class="relative w-full min-h-[180px] max-h-[min(60dvh,520px)] touch-manipulation overflow-auto overscroll-contain rounded-xl border border-purple-900/40 bg-[radial-gradient(circle,_rgba(255,255,255,0.12)_1px,_transparent_1px)] bg-[size:16px_16px] p-2">
    ...
    return {
        ...
        layoutSeatFaceStyle(el) {
            ...
            if (this.isSelectedSeat(el)) {
                return 'background-color:#ff2daa;border-color:#fecaca;color:#fff;box-shadow:0 0 0 2px #fff, 0 0 0 5px rgba(255,45,170,0.95);opacity:1;';
            }
